#!/usr/bin/env python3
"""
Build and push Docker/Podman images with smart tag generation.
"""
...
def main():
    """Main function."""
    try:
        print("🚀 Starting image build and push process...")
        ...
        push_image(registry_url, container_tool, built_tag)
        push_latest(registry_url, container_tool)
        
        print()
        print("🎉 Success! Image built and pushed with tags:")
        print(f"   - {registry_url}:{built_tag}")
        print(f"   - {registry_url}:latest")
        
    except KeyboardInterrupt:
        print("\n❌ Build and push cancelled by user")
        sys.exit(1)
